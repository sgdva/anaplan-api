...
class AuthorizationManager:
    ...
    def create_auth(self, method: str, **kwargs):
        ...
        TxtPassword = kwargs.get('TxtPassword')
        TxtPassword ="" if TxtPassword is None else TxtPassword
        TxtFilePathBin = kwargs.get('TxtFilePathBin')
        TxtFilePathkey = kwargs.get('TxtFilePathkey')
        post_data = None
        if method != "basic":
            post_data = authenticator.generate_post_data(private_key=private_key, TxtPassword=TxtPassword, TxtFilePathBin=TxtFilePathBin, TxtFilePathkey=TxtFilePathkey)
            #post_data = authenticator.generate_post_data(private_key=kwargs['private_key'], TxtPassword=kwargs['TxtPassword'], TxtFilePathBin=kwargs['TxtFilePathBin'], TxtFilePathkey=kwargs['TxtFilePathkey'])
