...
class TaskController:
    ...
    def __init__(
        ...
        self.IsErrorDumpNeeded=IsErrorDumpNeeded
    ...
    def _execute(self) -> ActionResponse:
        ...
        for response in self._parser_responses:
            if response.dump and self.IsErrorDumpNeeded==True:
                dumps.append(self.get_error_dump(endpoint=response.task_endpoint,TxtObjectID=response._raw_response['objectId']))
                pass
            if response.file:
                files.append(
                    FileDownload(
    ...
    def get_error_dump(self, endpoint: str, TxtObjectID:str="") -> DataFrame:
        ...
        """
        post_header = {
            "Authorization": self._conn.authorization._auth_token._token_value,
        ...
        try:
            ...
            TxtRealEndPoint= "/dumps/" + TxtObjectID + "/"
            TxtRealEndPoint = TxtRealEndPoint if TxtObjectID != "" else ""
            dump = self._handler.make_request(
                f"{endpoint}"+ TxtRealEndPoint, "GET", headers=post_header
            ).text
        ...
        try:
            ...
            logger.error("Err01TaskController.pyget_error_dump: AP Server threw an invalid dump. A workaround applying | as sep to continue exec was applied: bad lines skipped, warning!")
            edf = pd.read_csv(StringIO(dump),sep='|',on_bad_lines='skip')
