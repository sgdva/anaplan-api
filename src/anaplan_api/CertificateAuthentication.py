...
class CertificateAuthentication(AnaplanAuthentication):
	...
	def generate_post_data(priv_key: bytes,TxtPassword: str ="",TxtFilePathBin: str ="",TxtFilePathkey: str ="") -> str:
		...
		signed_nonce = str(CertificateAuthentication.sign_string(unsigned_nonce, priv_key,TxtPassword,TxtFilePathBin,TxtFilePathkey))
	...
	# ===========================================================================
	@staticmethod
	def sign_string(message: bytes, priv_key: bytes,TxtPassword: str ="",TxtFilePathBin: str ="",TxtFilePathkey: str ="") -> str:
		...
		try:
			if not os.path.isfile(priv_key):
				...
				with open(priv_key, 'r') as key_file:
					key = serialization.load_pem_private_key(open(priv_key, 'r').read().encode('utf-8'), TxtDecodedPass, backend=backend)
			try:
